...
class FrozenLake(object):
    ...
    #def value_iteration(self, threshold=0.001):
        ...
        while maxUtilityChange < threshold:
            maxUtilityChange = 0
            for i in range(self.height):
                for j in range(self.width):
                    s = (i, j)
                    if s in self.states:
                        maxUtilityValue = float("-inf")
                        for a in self.actions:
                            nextValue = 0
                            transitions = self.get_transitions(s, a)
                            print("Current State: ", s, "Transitions: ")
                            for transition in transitions:
                                if transition[0] in self.states:
                                    ...
                                    if state in self.holes:
                                        ...
                                        reward = -0.1
                                    nextValue += probability * (reward + self.gamma*values[state])
                            if nextValue > maxUtilityValue:
                                maxUtilityValue = nextValue
                        if abs(values[s] - maxUtilityValue) > 0:
                            maxUtilityChange = abs(values[s] - maxUtilityValue)
                        values[s] = maxUtilityValue
                        
                        print(maxUtilityValue, maxUtilityChange)
        return values
    """
    ...
    def value_iteration(self, threshold=0.001):
        ...
        values = dict((state, 0.0) for state in self.states)
        ### YOUR CODE HERE ###
        utilityChangePassThreshold = set()
        maxUtilityChange = 0
        while len(utilityChangePassThreshold) < len(self.states):
            for s in self.states:
                maxValue = float("-inf")
                for a in self.actions: #want best actions
                    nextValue = 0
                    for t in self.get_transitions(s, a):
                        if t[0] in self.targets:
                            reward = self.target_reward
                            nextValue += t[1] * (reward + self.gamma * self.target_reward)
                        elif t[0] in self.holes:
                            reward = self.hole_reward
                            nextValue += t[1] * (reward + self.gamma * self.hole_reward)
                        else:
                            reward = self.living_reward
                            nextValue += t[1] * (reward + self.gamma * values[t[0]])
                    if nextValue > maxValue:
                        maxValue = nextValue
                if abs(values[s] - maxValue) > 0:
                    maxUtilityChange = abs(values[s] - maxValue)
                    if maxUtilityChange < threshold:
                        utilityChangePassThreshold.add(s)
                values[s] = maxValue
        return values
    ...
    def Qlearner(self, alpha, epsilon, num_robots):
        ...
        for robot in range(num_robots):   
            ...
            action = None
            while current_state not in self.targets and current_state not in self.holes:
                if random.random() > epsilon:
                    # choose best
                    max_q = float("-inf")
                    best_a = None
                    for a in self.actions:
                        qvalue = Qvalues[(current_state, a)]
                        if qvalue > max_q:
                            max_q = qvalue
                            best_a = a
                    action = best_a
                ...
                if new_state in self.targets:
                    reward = self.target_reward
                    update_q = (1-alpha) * Qvalues[(current_state, action)] + alpha * (reward + self.gamma * self.target_reward)
                    Qvalues[(current_state, action)] = update_q
                elif new_state in self.holes:
                    reward = self.hole_reward
                    update_q = (1-alpha) * Qvalues[(current_state, action)] + alpha * (reward + self.gamma * self.hole_reward)
                    Qvalues[(current_state, action)] = update_q
                else:
                    reward = self.living_reward
                    new_max_q = float("-inf")
                    for a in self.actions:
                        transition = self.move(new_state, a)
                        if transition in self.targets or transition in self.holes or transition in self.states:
                            if transition in self.targets:
                                qvalue = self.target_reward
                            elif transition in self.holes:
                                qvalue = self.hole_reward
                            else:
                                qvalue = Qvalues[(new_state, a)]
                            if qvalue > new_max_q:
                                new_max_q = qvalue
                    update_q = (1-alpha) * Qvalues[(current_state, action)] + alpha * (reward + self.gamma * new_max_q)
                    Qvalues[(current_state, action)] = update_q
